...
![自定义主题](https://raw.githubusercontent.com/mike652638/opencode-zh-desktop/main/assets/custom-theme.png)
...
主题 JSON 使用 OpenCode Desktop theme 的 `name`、`id`、`light`、`dark` 结构，并保留 `palette`、`seeds`、`overrides` 和 `v2Overrides` 扩展字段。`theme detect` 会报告以下候选目录的实际存在状态，不会假设当前 OpenCode 版本一定支持某个目录：
